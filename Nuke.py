...
class projectile: # everything is mesured in Km unless stated otherwise
  def __init__(self, name, totalMoves, xCords, yCords, numberOfProjectile, blastRadius, eta, owner):
...
class ICBM(projectile): # everything is mesured in Km unless stated otherwise
  def __init__(self, name, totalMoves, xCords, yCords, numberOfProjectile, blastRadius, eta, owner):
    self.totalMoves = totalMoves 
    self.numberOfProjectile = numberOfProjectile
    self.xCords = xCords
...
def check_Projectile_Collision(): # will check Collision with a building if true output ->
    ...
    if (projectile.xCords == Radar.xCords) and (projectile.yCords == Radar.xCords):
