...
class Projectile:
    def __init__(self, name, power, xCords, yCords, speed, direction, owner):
        self.name = name
        self.power = power
        self.xCords = xCords
        self.yCords = yCords
...
def set_america():
    ...
    for x in range(0, 4, 1):
        ...
        america = Country('USA', 60000, 500,0)
        if Locations[x] == "Capital":
...
def display_america(): 
    ...
    print("\nLaunched Nuke")
    global owner  
    nuke = Projectile("Nuke", 5, 0,0, 1, 5,owner)
    ...
    for x in range(nuke.eta, nuke.totalMoves):
        ...
        nuke.xcords, nuke.Xcords = ycords, Xcords
...
def launch_ICBM(ICBM, owner):
    ...
    for x in range(ICBM.eta, ICBM.totalMoves): 
       ICBM.yCords = ypositions[x]
       ICBM.xCords = xpositions[x]
       ICBM.xCords = Xcords
       ICBM.YCords = Ycords
        # test_Player() # For testing purposes 
       player_Picker() # For the real game 
...
def set_ICBM():
    global Projectile, Xcords,Ycords
    owner = "test"
    Xcords = 500
    Ycords = 500
    Projectile("ICBM", 5, Xcords, Ycords, 1, 0.5, owner)
...
def check_projectile_Collision(): # will check Collision with a building if true output ->
    # Location[x] was hit if false -> you hit dirt 
    print("ahhhhhhhhhhhhhhhhhhhhh so many people died ooooooooo noooooooooooooooooooooooooo")
    if (Projectile.xCords == Radar.xDords) and (Projectile.Ycords == Radar.Cords):
        print("ahhhhhhhhhhhhhhhhhhhhh people died ooooooooo noooooooooooooooooooooooooo")
    else:                                                                  
        print("ahhhhhhhhhhhhhhhhhhhhh so many people died ooooooooo noooooooooooooooooooooooooo")
